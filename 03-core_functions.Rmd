...
<div class=semiemph>
All of the core dplyr functions take the following form:</div>
...
- It turns out that items that are labeled Q1 measured conscientiousness and items that are measured Q2 measure job performance. 
...
### Using mutate()
- Given that their are two sub scales (i.e., conscientiousness and performance) within our survey data, we can create scale scores for these sets of items. 
...
- We can now calculate each employee's team's average performance, conscientiousness, and team size in the data.
...
```{r, echo = FALSE, fig.height=1, fig.width=8}
ex_data%>%
  group_by(Manager)%>%
  summarise(team_cons = mean(cons, na.rm = TRUE), team_perf = mean(perf, na.rm = TRUE),
            sd_cons = sd(cons, na.rm = TRUE), sd_perf = sd(perf, na.rm = TRUE))%>%
