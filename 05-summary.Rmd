...
- Note that manager information was not originally stored in this data - two data sets had to be merged with this data frame.
...
- anit_join(): Reains rows in the left hand data frame that do NOT match and ID variable in the right hand data frame. 
...
- bind_cols is used when you have a set of data frames that 
    ...
    2. Are ordered identically, with no missing information
    
- If these two requirements are not met, the data will joined combining information about different participants
- If your data does not meet either of these requirements, use a join function discussed below.
...
- Unlike bind_cols(), bind_rows() attempts to match columns based on their name
