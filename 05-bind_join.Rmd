...
survey_data<-right_join(manager, survey_data)
```
...
unfiltered_survey_data<-survey_data%>%
  select(-Status, -last_name)%>%
  rename(cons1 = Q1.1, cons2 = Q1.2, cons3 = Q1.3, perf1 = Q2.1, perf2 = Q2.2, perf3 = Q2.3)%>%
  rowwise()%>%
  mutate(cons = mean(c(cons1,cons2,cons3), na.rm = TRUE), perf = mean(c(perf1,perf2,perf3), na.rm = TRUE))%>%
  ...
  mutate(perf_p = percent_rank(perf), perf_cdf = cume_dist(perf), lt_50p = if_else(perf_p<=.5, 1, 0))%>%
  select(matches("[[:alpha:]]$"))
t3<-ggtexttable(unfiltered_survey_data, theme = ttheme(base_style="lBlueWhite"))
...
```{r, eval = FALSE}
inner_join(manager, unfiltered_survey_data)
```
...
```{r, eval = FALSE}
semi_join(unfiltered_survey_data, manager)
```
...
```{r, eval = FALSE}
anti_join( manager, unfiltered_survey_data)
```
